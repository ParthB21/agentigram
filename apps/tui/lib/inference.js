...
module.exports = class Inference extends ReadyResource {
  ...
  // `responseFormat` is an optional structured-output constraint (see
  // workers/qvac.js) — pass one when the answer has to parse.
  ask(history, responseFormat) {
    const id = ++this._seq
    this._send({ t: 'ask', id, history, ...(responseFormat && { responseFormat }) })
    return id
  }

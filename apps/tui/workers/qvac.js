...
async function ask(id, history, responseFormat) {
  ...
  //
  // responseFormat, when given, is a json_schema constraint that llama.cpp
  // compiles to GBNF for this request only. Agentigram uses it for the contract
  // draft, where the output has to parse.
...
pipe.on('data', (data) => {
  ...
  if (msg.t === 'ask') ask(msg.id, msg.history, msg.responseFormat).catch(noop)

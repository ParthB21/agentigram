...
- Writes are narrated by the daemon (`write-narration.ts`) rather than one line per save: an agent says what it is working on, then goes quiet for 8s, then names everything saved in the meantime — "Changing user.ts, session.ts and three other files." A coding agent saves far faster than a voice reads, and one line per save would evict the negotiation from an eight-line queue. Those lines are priority 0, so a collision takes the speaker back immediately. The engineer's own hand edits, which the watcher also reports, are never read back.

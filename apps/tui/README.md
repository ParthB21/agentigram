...
- Only fresh events are considered. Exact `MESSAGE` text, deterministic coordination milestones, an agent arriving or leaving, and what it is working on are eligible; heartbeats, file reads, tool calls, logs, and persona filler are silent.
- Writes are narrated by the daemon (`speech-narration.ts`) rather than one line per save: an agent says what it is working on, then goes quiet for 8s, then names everything saved in the meantime — "Changing user.ts, session.ts and three other files." A coding agent saves far faster than a voice reads, and one line per save would evict the negotiation from an eight-line queue. Those lines are priority 0, so a collision takes the speaker back immediately. The engineer's own hand edits, which the watcher also reports, are never read back.
- One arrival is one announcement: a session that ends twice (its own farewell, then the authority noticing the socket close) is spoken once, even though the feed shows both. The rule is the same sentence twice within 30s, not "the room already lists that agent" — restarting the authority restores a roster naming everyone who was present, and the agents that reconnect into it have not yet been introduced to anybody listening.

...
- 2026-09-20 · half 1 · Presence is speakable, in the arriving agent's own voice, and deduplicated by repeating sentence within 30s rather than against the room state · an arrival means that agent's writes can now collide with yours, while its reads and tool calls remain telemetry. The state check silenced the common case instead: a restarted authority restores a roster that still lists everyone, so every agent reconnecting into it looked like it was already here and was never announced.
...
- 2026-09-20 · half 1 · Writes are narrated per agent on an 8s cooldown by `daemon/speech-narration.ts`, not restored to `SPEAKABLE_TYPES` · `FILE_WRITE` was dropped from that set in 4bd215c because one line per save floods an eight-line queue; collapsing a burst into one sentence needs memory of the last few writes, which a pure per-event renderer does not have. Narration state is per-laptop presentation, so there is nothing for two laptops to disagree about.
...
- 2026-09-19 · half 1 · The Electron window exposes only four narrow preload methods; renderer Node integration stays disabled and sandboxing/context isolation remain enabled · live local status does not justify a privileged renderer.
...
- 2026-09-20 · part 1 (daemon) · The authority writes a scored session report (`report.json` + `report.md`) to `~/.agentigram/reports/` on shutdown, and `agg report` rebuilds one from any `events.jsonl` · every number is a count over the event log (CI, contract, run verification), never a question bank, so the same log always gives the same score and the report carries the log hash. Sessions with no verification signal are `unverified` and excluded from rates, not counted as failures. Reports sit outside p2p storage because `agg leave` deletes that.
